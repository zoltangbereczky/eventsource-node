 //Modified for Spark BxLab project. Emits 'Event' not the real event type. So we can subscribe to all events with .onEvent
 // Internal Analytics
 var keenLogger =  require("../../../lib/keenLogger");
...
function EventSource(url, eventSourceInitDict) {
  ...
  function onConnectionClosed() {
	...
	keenLogger.addEvent("connectionClosed");
    ...
    if (lineLength === 0) {
      if (data.length > 0) {
        ...
        _emit(type, new MessageEvent(type, {
	      ...
	      ///BGZ MOD for all events
		  _emit('Event', new MessageEvent(type, {
		      data: data.slice(0, -1), // remove trailing newline
		      lastEventId: lastEventId,
		      origin: original(url)
		  }));
...
['open', 'error', 'message', 'Event'].forEach(function (method) {
